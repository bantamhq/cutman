...
pub fn content_router() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route("/repos/{id}/commits/{sha}", get(handlers::get_commit_handler))
        .route("/repos/{id}/commits/{sha}/diff", get(handlers::get_commit_diff))
        .route("/repos/{id}/compare/{spec}", get(handlers::compare_refs))
        .route("/repos/{id}/tree/{ref}", get(handlers::get_tree_root))
        .route("/repos/{id}/tree/{ref}/{*path}", get(handlers::get_tree_handler))
